...
for epoch_i in range(num_train_instances):
 ...
 ########################################################################################################
    ...
    if apply_masking:
        ...
        sim_fwgt_mask[epoch_i,:,:,:], mask[epoch_i,:,:,:] = apply_random_brain_mask(sim_fwgt[epoch_i,:,:,:])
...
######################
    if backgroundfield:
          ...
          sim_fwgt_mask_bg[epoch_i,:,:,:] = tf.add(sim_fwgt_mask[epoch_i,:,:,:], bgf_mask[epoch_i,:,:,:]) #????????????dkjfsdfsdsdklfjdsjfhhsdgfkjsdfkllfdfjfk
...
######################          
    if sensor_noise:
        ...
        sim_fwgt_mask_bg_sn[epoch_i, :,:,:] = tf.add(sim_fwgt_mask_bg[epoch_i, :,:,:], tf_noise.numpy()) 
  ...
  ######################                
    if wrap_input_data:
         ...
         sim_fwgt_mask_bg_sn_wrapped[epoch_i, :,:,:] = tf.add(sim_fwgt_mask_bg_sn[epoch_i, :,:,:], value_range / 2.0)
         ...
         sim_fwgt_mask_bg_sn_wrapped[epoch_i, :,:,:] = tf.math.floormod(sim_fwgt_mask_bg_sn_wrapped[epoch_i, :,:,:], value_range)
         Xongoing[epoch_i, :,:,:] = tf.math.floormod(Xongoing[epoch_i, :,:,:], value_range)
     
         
                  # shift back to [-pi,pi]
         sim_fwgt_mask_bg_sn_wrapped[epoch_i, :,:,:] = tf.subtract(sim_fwgt_mask_bg_sn_wrapped[epoch_i, :,:,:], value_range / 2.0)
 ...
 ######################          
    if apply_masking:
        ...
        sim_fwgt_mask_bg_sn_wrapped[epoch_i, :,:,:] = np.multiply(mask[epoch_i, :,:,:], sim_fwgt_mask_bg_sn_wrapped[epoch_i, :,:,:])
...
view_slices_3dNew(sim_fwgt[index,:,:,:], 50,50,50, vmin=-1.5, vmax=1.5, title="fw")


view_slices_3dNew(Xinput[index,:,:,:], 50,50,50, vmin=-1.5, vmax=1.5, title="Xinput (fw+mask)")
view_slices_3dNew(sim_fwgt_mask[index,:,:,:], 50,50,50, vmin=-0.5, vmax=0.5, title="fw+brain mask")
...
view_slices_3dNew(sim_fwgt_mask_bg[index,:,:,:], 50,50,50, vmin=-10, vmax=10, title="fw + mask +background")
view_slices_3dNew(sim_fwgt_mask_bg[index,:,:,:], 50,50,50, vmin=-0.5, vmax=0.5, title="fw + mask +background+boundarz artifacs")



view_slices_3dNew(sensornoise[index,:,:,:], 50, 50,50, vmin=-0.5, vmax=0.5, title="sn")

view_slices_3dNew(sim_fwgt_mask_bg_sn[index,:,:,:], 50, 50,50, vmin=-10, vmax=10, title="fw+bg + mask (bondary artifacts)+sn")


view_slices_3dNew(sim_fwgt_mask_bg_sn_wrapped[index,:,:,:], 50, 50,50, vmin=-10, vmax=10, title="bg field + masking (bondary artifacts)+wrapped")
...
#loaded_array = np.load('phase_bg.npy')

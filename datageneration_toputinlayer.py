...
from backgroundfieldandeffects.functionsfromsteffen import apply_random_brain_mask_tf
from backgroundfieldandeffects.generate_backgroundfield_steffen_function import add_z_gradient_tf
...
for epoch_i in range(num_train_instances):
    ...
    sim_fwgt[epoch_i, :, :, :] =  tf.convert_to_tensor(forward_convolution(sim_gt[epoch_i, :, :, :]))
    sim_gt[epoch_i, :, :, :] =  tf.convert_to_tensor(sim_gt[epoch_i, :, :, :])
 ...
 ########################################################################################################
    ...
    if apply_masking:
        ...
        sim_fwgt_mask[epoch_i, :, :, :], mask[epoch_i, :, :,
                                              :] = apply_random_brain_mask_tf(tf.convert_to_tensor(sim_fwgt[epoch_i, :, :, :]))
...
######################
 ...
 ######################
    if apply_masking:
        ...
        sim_fwgt_mask_bg_sn_wrapped[epoch_i, :, :, :]  = tf.multiply(
            tf.to_float(mask[epoch_i, :, :, :]), sim_fwgt_mask_bg_sn_wrapped[epoch_i, :, :, :])

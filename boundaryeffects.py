...
# brain_area = self.__dilation(mask, tf.ones([5, 5, 5]), [5, 5, 5],
 #                              shape, 1)
...
#    selected_part = tf.logical_xor(dilation_result, brain_area)
    ...
    #result = tf.multiply(tf.to_float(tf.logical_not(brain_area)),
   #                      artifacts)
    ...
    #result = tf.add(np.float32(data), result)
    result = tf.add(data, np.float64(result))

 
    # apply mask again
    #result = tf.multiply(tf.to_float(mask), result)
...
X = data
##
view_slices_3d(X, slice_nbr=50, vmin=-0.5, vmax=0.5, title="data X")
####
...
if backgroundfield:
                ...
                view_slices_3d(bgf, slice_nbr=50, vmin=-20, vmax=20, title="bg field")
                ...
                if apply_masking:
                    ...
                    view_slices_3d(bgf, slice_nbr=50, vmin=-20, vmax=20, title="bg field + masking")

                                

                X = tf.add(X_input, bgf) #????????????dkjfsdfsdsdklfjdsjfhhsdgfkjsdfkllfdfjfk
                
                
                view_slices_3d(X, slice_nbr=50, vmin=-20, vmax=20, title="bg field + masking")
...
if sensor_noise:
    ...
    view_slices_3d(X, slice_nbr=50, vmin=-50, vmax=50, title="noise+X")

...
## alternative
   ...
   #plt.show()
##########################
   plt.imshow(phase[64,:,:], cmap='gray',  vmin=-0.2, vmax=0.2)  
   plt.colorbar()
...
#####################################################
   ...
   plt.imshow(mask[64,:,:], cmap='gray',  vmin=-0.2, vmax=0.2)   
   plt.colorbar()
   max_o = str(round(np.max(mask),2))
   min_o = str(round(np.min(mask),2))
   plt.title("file " +  str(epoch_i) + ': phasebg - max: '+ max_o + ' min ' + min_o)
   plt.show()

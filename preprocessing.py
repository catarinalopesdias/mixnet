...
for epoch_i in range(num_epochs):
    sim_gt_full[epoch_i,:,:,:] = simulate_susceptibility_sources(simulation_dim = size, rectangles_total = 80)
    # forward convolution with the dipole kernel 
    sim_fw_full[epoch_i,:,:,:]  = forward_convolution(sim_gt_full)
...
print("fit model")
history = model.fit(train_images, train_labels,  epochs=epochs_train, batch_size=1, shuffle=True,

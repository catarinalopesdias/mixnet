...
#newadam16cp-4947_trainsamples150_datasetiter5000_batchsize1_gaaccum10_loss_mse_1gt150backgrounds.ckpt
...
path = "checkpoints/bgremovalmodel/newadam"+str(num_filter)+"cp-"+ str(finalcheckpoint) + \
    "_trainsamples" + str(num_train_instances) + "_datasetiter" + str(dataset_iterations) + \
        "_batchsize" + str(batch_size)+ "_gaaccum" + str(gaaccumsteps) + "_loss_" + lossU+"_1gt150backgrounds.ckpt"
...
# compressed data
if newdata:
    loaded = np.load('datasynthetic/10backgroundfields_1gt_lessbg.npz')
#np.load('datasynthetic/5samples.npz')
    text = "testdata"
else: #traindata
...
for epoch_i in range(2): #num_instance
    ...
    print(epoch_i)
    title =   "trained network. " + text + str(epoch_i)+ " " + lossU
    pathi = "models/backgroundremovalBOLLMAN/results/train"+str(num_filter) + "trainsamples" + \
        str(num_train_instances) + "_datasetiter" + str(dataset_iterations) + "_batchsize" + str(batch_size)+\
            "_gaaccum" + str(gaaccumsteps) + "_loss_" + lossU+ "_testset_epoch" + str(epoch_i) + text +"1gt150bgcolor"
    predicted, reference,error = visualize_all4(phasebg[epoch_i,:,:,:], phase[epoch_i,:,:,:], y_pred[0,:,:,:,0] ,title = title , save = True, path = pathi )
    
    pathi = "models/backgroundremovalBOLLMAN/results/train"+str(num_filter) + "trainsamples" + \
        str(num_train_instances) + "_datasetiter" + str(dataset_iterations) + "_batchsize" + str(batch_size)+\
            "_gaaccum" + str(gaaccumsteps) + "_loss_" + lossU+ "_testset_epoch" + str(epoch_i) + text +"1gt150bggrey"

...
def build_CNN_phillip(input_tensor, initial_filt):


#########################################################################################################################
    # First convolutional layer with activation and batch normalization
    X = Conv3D(filters=initial_filt, kernel_size=[3, 3, 3],padding='same')(input_tensor)
    ...
    X = Conv3D(filters=initial_filt, kernel_size=[3, 3, 3],padding='same')(X)
    X = LeakyReLU(alpha=0.2)(X)
    X = BatchNormalization()(X)
    ...
    encoding_down_1 = Conv3D(filters=initial_filt,
                            kernel_size=[3, 3, 3],
...
#############################################################################################################################
    #Second Layer-->down
    # First convolutional layer with activation and batch normalization
    X = Conv3D(filters=initial_filt*2, kernel_size=[3, 3, 3],padding='same')(batch_norm_layer_3)
    X = LeakyReLU(alpha=0.2)(X)
    X = BatchNormalization()(X)
    ...
    # Second convolutional layer with activation and batch normalization
    X = Conv3D(filters=initial_filt*2, kernel_size=[3, 3, 3],padding='same')(X)
    X = LeakyReLU(alpha=0.2)(X)
    X = BatchNormalization()(X)
    ...
    encoding_down_2 = Conv3D(filters=initial_filt*2,
                            kernel_size=[3, 3, 3],
                            activation=LeakyReLU(alpha=0.2),
                            padding='same',
                            kernel_initializer=GlorotNormal(42),
...
###################################################################################################################################################
    #Third Layer-->down
    # First convolutional layer with activation and batch normalization
    X = Conv3D(filters=initial_filt*4, kernel_size=[3, 3, 3],padding='same')(batch_norm_layer_2)
    X = LeakyReLU(alpha=0.2)(X)
    X = BatchNormalization()(X)
    ...
    # Second convolutional layer with activation and batch normalization
    X = Conv3D(filters=initial_filt*4, kernel_size=[3, 3, 3],padding='same')(X)
    X = LeakyReLU(alpha=0.2)(X)
    X = BatchNormalization()(X)
    ...
    encoding_down_3 = Conv3D(filters=initial_filt*4,
                            kernel_size=[3, 3, 3],
                            activation=LeakyReLU(alpha=0.2),
                            padding='same',
                            kernel_initializer=GlorotNormal(42),
...
#######################################################################################################################
    ...
    X = Conv3D(filters=initial_filt*8, kernel_size=[3, 3, 3],padding='same')(batch_norm_layer_3)
    X = LeakyReLU(alpha=0.2)(X)
    X = BatchNormalization()(X)

    X_save = X

    X = Conv3D(filters=initial_filt*8, kernel_size=[3, 3, 3],padding='same')(X)
    X = LeakyReLU(alpha=0.2)(X)
    X = BatchNormalization()(X)
...
#####################################################################################################
    ...
    decoder_1 = Conv3DTranspose(filters=initial_filt*4,
                        kernel_size=[3,3,3],
                        activation=LeakyReLU(alpha=0.2),
    ...
    X = Conv3D(filters=initial_filt*4, kernel_size=[3, 3, 3],padding='same')(combine_conc1_dec1)
    X = LeakyReLU(alpha=0.2)(X)
    X = BatchNormalization()(X)
...
##########################################################################################################
    ...
    decoder_2 = Conv3DTranspose(filters=initial_filt*2,
                        kernel_size=[3,3,3],
                        activation=LeakyReLU(alpha=0.2),
    ...
    X = Conv3D(filters=initial_filt*2, kernel_size=[3, 3, 3],padding='same')(combine_conc2_dec2)
    X = LeakyReLU(alpha=0.2)(X)
    X = BatchNormalization()(X)
...
########################################################################################################################
    ...
    decoder_3 = Conv3DTranspose(filters=initial_filt,
                        kernel_size=[3,3,3],
                        activation=LeakyReLU(alpha=0.2),
    ...
    X = Conv3D(filters=initial_filt, kernel_size=[3, 3, 3],padding='same')(combine_conc3_dec3)
    X = LeakyReLU(alpha=0.2)(X)
    X = BatchNormalization()(X)

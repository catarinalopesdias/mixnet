...
gradient_slope_range = [1 * 2 * np.pi, 4 * 2 * np.pi]
...
simulation1 = simulate_susceptibility_sources(
...
for epoch_i in range(num_train_instances):
    ...
    sim_gt[epoch_i, :, :, :] = simulation1 #simulate_susceptibility_sources(
       # simulation_dim=size, rectangles_total=rect_num, plot=False)
    # Phase:forward convolution with the dipole kernel
    sim_fwgt[epoch_i, :, :, :] = phase1 #forward_convolution(sim_gt[epoch_i, :, :, :])
...
view_slices_3dNew(Xongoing[index, :, :, :], 50, 50, 50,
                  vmin=-2, vmax=2, title="X ongoing-fw,masj,bg,sn,wrapped")
############################
...
title = "datasynthetic/" + str(num_train_instances) + "backgroundfields_1gt_lessbg"

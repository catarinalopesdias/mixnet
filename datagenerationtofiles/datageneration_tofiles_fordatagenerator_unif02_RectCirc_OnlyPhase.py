...
for epoch_i in range(num_train_instances):
    ...
    if testingdata:
       file_name = "datasynthetic/uniform02RectCircleOnlyPhase/testing/" + str(epoch_i) + "samples"
    else:
       file_name = "datasynthetic/uniform02RectCircleOnlyPhase/training/" + str(epoch_i) + "samples"
         
    arr  = np.stack((mask,sim_fwgt_mask,sim_fwgt), axis=0)
